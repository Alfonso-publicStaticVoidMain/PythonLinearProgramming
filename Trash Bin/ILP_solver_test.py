...
def solve_assignment_problem(
    ...
    # Deduce the number of workers and tasks from the efficiency matrix
    workers: int = len(efficiency)
    tasks: int = len(efficiency[0]) if efficiency else 0
    demanded_workers: int = sum(demand)
...
demand = [4, 3, 5, 4, 2, 3, 2, 3, 2, 2]


assignment = solve_assignment_problem(efficiency, demand, True)